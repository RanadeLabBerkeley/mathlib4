...
noncomputable def DecisionTreeComplexity (f : BoolFn n) : ℕ :=
  sInf { d : ℕ | ∃ t : DecisionTree n, t.computes f ∧ t.depth = d }
...
def pathLength (t : DecisionTree n) (x : BoolInput n) : ℕ :=
  match t with
  | DecisionTree.leaf _ => 0
  | DecisionTree.node i l r => 1 + (if x i then (pathLength r x) else (pathLength l x))
...
lemma pathLength_le_depth (t : DecisionTree n) (x : BoolInput n) :
  ...
  | node i l r ih_l ih_r =>
    simp [pathLength, DecisionTree.depth]
...
theorem sensitivityLeDecisionTreeComplexity {t : DecisionTree n} {f : BoolFn n}
   ...
   apply Finset.sup_le
   intro x hx
   calc sensitivityAt f x ≤ pathLength t x := by exact sensitivityAt_le_pathLength h_comp x
        _ ≤ t.depth := by exact pathLength_le_depth t x
