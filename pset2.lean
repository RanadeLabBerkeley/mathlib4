...
theorem mem_language_iff {α : Type} (M : DFA α) (w : List α) :
  w ∈ M.language ↔ accepts M w = true := by
  constructor
  ·
    intro h
    unfold DFA.language at h
    assumption
  ·
    intro h
    unfold DFA.language
...
theorem endsWithI_accepts_suffix_I (w : List Bit) :
  ...
  simp [accepts, endsWithI]
  exact endsWithI_run_ends_with_I w
...
theorem endsWithI_rejects_suffix_O (w : List Bit) :
  ...
  simp [accepts, endsWithI]
  exact endsWithI_run_ends_with_O w
...
theorem endsWithI_language_correct :
  ...
  ·
    intro h
    unfold DFA.language at h
    unfold endsWithI_lang
    simp [mem_language_iff]
  ·
    simp [DFA.language, endsWithI_lang, endsWithIWord, mem_language_iff]
