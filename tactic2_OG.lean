...
elab "bigO" : tactic => withMainContext do
  ...
  try
    ...
    -- Step 2: Prove ∀ n : ℝ, canonicalLHS n = f n (by ring)
    ...
    -- Solve the eq goal using intro + ring via tactic evaluation
    let savedGoals ← getGoals
    setGoals [eqMVar.mvarId!]
    try
      evalTactic (← `(tactic| intro n; ring_nf))
    catch _ =>
      setGoals savedGoals
      throw directErr
    setGoals savedGoals
    ...
    -- Step 3: Apply IsBigO.congr_left to get f =O[atTop] g
