...
/-- n^k =O(n^k * (Real.log n)^b) when b ≥ 1.
    Covers the case where LHS has no log factor but RHS does. -/
theorem bigO_poly_polyMulLogPow {k b : ℕ} (hb : 1 ≤ b) :
    (fun n : ℝ => n ^ k) =O[atTop] (fun n => n ^ k * (Real.log n) ^ b) := by
  have : (fun n : ℝ => n ^ k) = (fun n => n ^ k * (Real.log n) ^ 0) := by simp
  rw [this]
  exact bigO_polyMulLog_polyMulLog (by omega)

/-- n =O(n * (Real.log n)^b) when b ≥ 1. Bare-variable variant. -/
theorem bigO_id_idMulLogPow {b : ℕ} (hb : 1 ≤ b) :
    (fun n : ℝ => n) =O[atTop] (fun n => n * (Real.log n) ^ b) := by
  have h := bigO_poly_polyMulLogPow (k := 1) hb
  simp only [pow_one] at h
  exact h

/-- n =O(n * Real.log n). Bare-log-and-variable variant. -/
theorem bigO_id_idMulBareLog :
    (fun n : ℝ => n) =O[atTop] (fun n => n * Real.log n) := by
  have h := bigO_id_idMulLogPow (b := 1) (by omega)
  simp only [pow_one] at h
  exact h

/-- n^k =O(n^k * Real.log n). Bare-log variant. -/
theorem bigO_poly_polyMulBareLog {k : ℕ} :
    (fun n : ℝ => n ^ k) =O[atTop] (fun n => n ^ k * Real.log n) := by
  have h := bigO_poly_polyMulLogPow (k := k) (b := 1) (by omega)
  simp only [pow_one] at h
  exact h
...
/-- Convert a GrowthTerm back to a Lean expression body (inverse of parseExpr
    for single terms). Produces canonical forms matching what parseExpr generates
    and proveLeaf expects: bare `n` not `n^1`, bare `log n` not `(log n)^1`. -/
...
/-- Build a lambda `fun n : ℝ => dominant_term(n)` from a GrowthExpr's dominant term.
    Reuses the original lambda's binder for structural compatibility. -/
private def dominantToLambda (ge : GrowthExpr) (originalLam : Expr) : MetaM Expr := do
  let dom := ge.dominant
  let tryBuild (e : Expr) : MetaM (Option Expr) := do
    match e with
    | .lam name ty _body bi =>
      withLocalDecl name bi ty fun fvar => do
        let body ← growthTermToExpr dom fvar
  ...
  withLocalDeclD `n (mkConst ``Real) fun var => do
    let body ← growthTermToExpr dom var
...
private def matchAdd? (e : Expr) : Option (Expr × Expr) :=
  ...
  | (``Add.add, args) =>
    if h : args.size ≥ 4 then some (args[2]!, args[3]!) else none
  | _ => none
...
partial def parseExpr (e : Expr) (var : Expr) : MetaM GrowthExpr := do
  ...
  if let some (base, exp) := matchPow? e then
    ...
    -- Nested power: (n^a)^b = n^(a*b), or (log n)^a)^b = (log n)^(a*b)
    if let some (innerBase, innerExp) := matchPow? base then
      if let some b ← extractNatLit? exp then
        if ← isDefEq innerBase var then
          if let some a ← extractNatLit? innerExp then
            return ⟨[{ polyExp := Int.ofNat (a * b), logExp := 0 }]⟩
        if let some logArg ← matchLog? innerBase then
          if ← isDefEq logArg var then
            if let some a ← extractNatLit? innerExp then
              return ⟨[{ polyExp := 0, logExp := Int.ofNat (a * b) }]⟩
    if let some logArg ← matchLog? base then
      if ← isDefEq logArg var then
        if let some m ← extractNatLit? exp then
          return ⟨[{ polyExp := 0, logExp := Int.ofNat m }]⟩
    -- General power of single-term base: (expr)^b where expr parses to one term
    if let some b ← extractNatLit? exp then
      let baseGrowth ← parseExpr base var
      if baseGrowth.terms.length == 1 then
        let t := baseGrowth.terms.head!
        return ⟨[{ polyExp := t.polyExp * Int.ofNat b, logExp := t.logExp * Int.ofNat b }]⟩
  ...
  if let some logArg ← matchLog? e then
    ...
    -- log(n^a) = a * log(n) = Θ(log n) for big-O purposes
    if let some (base, exp) := matchPow? logArg then
      if ← isDefEq base var then
        if (← extractNatLit? exp).isSome then
          return ⟨[{ polyExp := 0, logExp := 1 }]⟩
...
| .polyEqLogLe =>
  ...
  if fk == 0 then
    ...
    -- n^k * log^a ≤ n^k * log^b (same poly degree, different log degree)
    if fm == 0 then
      -- No log in LHS: n^k =O(n^k * (log n)^b)
      if fk == 1 then
        applyWithFallback goal
          ``bigO_poly_polyMulLogPow
          ``bigO_id_idMulLogPow
      else
        applyWithFallback goal
          ``bigO_poly_polyMulLogPow
          ``bigO_poly_polyMulBareLog
    else if fm == 1 then
      applyWithFallback goal
...
-- ═══════════════════════════════════════════════════════════════════════════
-- PART 4b: THETA PROOF CONSTRUCTION (for simproc rewriting)
-- ═══════════════════════════════════════════════════════════════════════════

/-- Build proof of `originalLam =Θ[atTop] simplifiedLam` where `originalLam` is
    a multi-term sum and `simplifiedLam` is its dominant term.
    Uses buildProof (forward: sum =O dominant) and proveDominantOfSum
    (backward: dominant =O sum), combined via IsBigO.antisymm. -/
private def buildThetaProof (originalLam simplifiedLam : Expr)
    (origGrowth : GrowthExpr) (domTerm : GrowthTerm) : MetaM Expr := do
  let realTy := mkConst ``Real
  let atTopExpr ← mkAppOptM ``Filter.atTop #[realTy, none]
  -- Forward: original =O[atTop] simplified
  let fwdType ← mkAppM ``Asymptotics.IsBigO #[atTopExpr, originalLam, simplifiedLam]
  let fwdMVar ← mkFreshExprMVar fwdType
  buildProof origGrowth.terms domTerm fwdMVar.mvarId!
  let fwdProof ← instantiateMVars fwdMVar
  -- Backward: simplified =O[atTop] original
  let bwdType ← mkAppM ``Asymptotics.IsBigO #[atTopExpr, simplifiedLam, originalLam]
  let bwdMVar ← mkFreshExprMVar bwdType
  proveDominantOfSum bwdMVar.mvarId!
  let bwdProof ← instantiateMVars bwdMVar
  -- Combine: IsBigO.antisymm fwdProof bwdProof → original =Θ simplified
  -- Then .symm to get simplified =Θ original (which is what congr_left/right expect)
  mkAppM ``Asymptotics.IsBigO.antisymm #[fwdProof, bwdProof]

-- ═══════════════════════════════════════════════════════════════════════════
-- PART 4c: SIMPROC (for rewriting bigO sub-expressions)
-- ═══════════════════════════════════════════════════════════════════════════

/-- Extract (l, f, g) from an `Asymptotics.IsBigO l f g` expression. -/
private def matchBigOExpr? (e : Expr) : MetaM (Option (Expr × Expr × Expr)) := do
  let extractLFG (e : Expr) : Option (Expr × Expr × Expr) :=
    let args := e.getAppArgs
    if args.size ≥ 3 then
      some (args[args.size - 3]!, args[args.size - 2]!, args[args.size - 1]!)
...
/-- Try to fully prove an IsBigO expression, returning a proof or none.
    Reuses the exact same proof strategy as the `bigO` tactic.
    For multi-term RHS, builds explicit intermediate with concrete types
    to avoid metavar resolution issues. -/
private def tryProveBigO (e : Expr) (_l f g : Expr) : MetaM (Option Expr) := do
  let some fGrowth ← parseLambdaToGrowth f | return none
  let some gGrowth ← parseLambdaToGrowth g | return none
  let gDom := gGrowth.dominant
  -- Check all LHS terms are dominated by RHS
  for t in fGrowth.terms do
    unless GrowthTerm.le t gDom do return none
  try
    if gGrowth.terms.length ≤ 1 then
      -- Single-term RHS: direct proof on original expression
      let proofMVar ← mkFreshExprMVar e
      buildProof fGrowth.terms gDom proofMVar.mvarId!
      let proof ← instantiateMVars proofMVar
      if proof.hasExprMVar then return none
      return some proof
    else
      -- Multi-term RHS: build f =O(domLam) and domLam =O(g) separately
      -- with concrete types, then combine via isBigO_trans_real.
      let realTy := mkConst ``Real
      let atTopExpr ← mkAppOptM ``Filter.atTop #[realTy, none]
      let domLam ← dominantToLambda gGrowth g
      -- First: f =O[atTop] domLam (concrete RHS, no metavars)
      let firstType ← mkAppM ``Asymptotics.IsBigO #[atTopExpr, f, domLam]
      let firstMVar ← mkFreshExprMVar firstType
      buildProof fGrowth.terms gDom firstMVar.mvarId!
      let firstProof ← instantiateMVars firstMVar
      if firstProof.hasExprMVar then return none
      -- Second: domLam =O[atTop] g (dominant embeds into sum)
      let secondType ← mkAppM ``Asymptotics.IsBigO #[atTopExpr, domLam, g]
      let secondMVar ← mkFreshExprMVar secondType
      proveDominantOfSum secondMVar.mvarId!
      let secondProof ← instantiateMVars secondMVar
      if secondProof.hasExprMVar then return none
      -- Combine via transitivity
      let proof ← mkAppM ``isBigO_trans_real #[firstProof, secondProof]
      return some proof
  catch _ => return none

/-- The simproc implementation. Matches any `IsBigO l f g` expression and
    tries to prove it completely, converting to True. Falls back to Θ-based
    simplification if full proof fails but sides can be simplified. -/
private def bigOSimprocImpl : Simp.Simproc := fun e => do
  let some (l, f, g) ← matchBigOExpr? e | return .continue
  -- Only handle atTop
  unless (match l.getAppFn with | .const n _ => n == ``Filter.atTop | _ => false) do
    return .continue

  -- Strategy 1: Try to fully prove the IsBigO → True
  if let some proof ← tryProveBigO e l f g then
    let iffTrue ← mkAppM ``iff_true_intro #[proof]
    let eqTrue ← mkAppM ``propext #[iffTrue]
    return .done { expr := mkConst ``True, proof? := some eqTrue }

  -- Strategy 2: Try Θ-based simplification (rewrite multi-term sides to dominant)
  let some fGrowth ← parseLambdaToGrowth f | return .continue
  let some gGrowth ← parseLambdaToGrowth g | return .continue
  let fDom := fGrowth.dominant
  let gDom := gGrowth.dominant
  let fMultiTerm := fGrowth.terms.length > 1
  let gMultiTerm := gGrowth.terms.length > 1
  if !fMultiTerm && !gMultiTerm then return .continue

  try
    -- Build simplified lambdas by opening original lambdas and reconstructing
    let fSimp ← if fMultiTerm then dominantToLambda fGrowth f else pure f
    let gSimp ← if gMultiTerm then dominantToLambda gGrowth g else pure g

    let realTy := mkConst ``Real
    let atTopExpr ← mkAppOptM ``Filter.atTop #[realTy, none]

    -- Build ↔ proof chain
    let mut currentIff : Option Expr := none
    let mut currentLHS := f
    let mut currentRHS := g

    if fMultiTerm then
      let thetaF ← buildThetaProof f fSimp fGrowth fDom
      let iffF ← mkAppOptM ``Asymptotics.IsTheta.isBigO_congr_left
        #[none, none, none, none, none, none, none, none, none, some thetaF, some currentRHS]
      currentIff := some iffF
      currentLHS := fSimp

    if gMultiTerm then
      let thetaG ← buildThetaProof g gSimp gGrowth gDom
      let iffG ← mkAppOptM ``Asymptotics.IsTheta.isBigO_congr_right
        #[none, none, none, none, none, none, none, none, none, some thetaG, some currentLHS]
      match currentIff with
      | none => currentIff := some iffG
      | some prevIff =>
        let combined ← mkAppM ``Iff.trans #[prevIff, iffG]
        currentIff := some combined
      currentRHS := gSimp

    let some iffProof := currentIff | return .continue
    let simplifiedExpr ← mkAppM ``Asymptotics.IsBigO #[atTopExpr, currentLHS, currentRHS]

    -- Try to fully discharge the simplified version too
    if let some directProof ← tryProveBigO simplifiedExpr l currentLHS currentRHS then
      let fullProof ← mkAppM ``Iff.mpr #[iffProof, directProof]
      let iffTrue ← mkAppM ``iff_true_intro #[fullProof]
      let eqTrue ← mkAppM ``propext #[iffTrue]
      return .done { expr := mkConst ``True, proof? := some eqTrue }

    -- Return partial simplification
    let eqProof ← mkAppM ``propext #[iffProof]
    return .visit { expr := simplifiedExpr, proof? := some eqProof }
  catch _ => return .continue

simproc_decl bigO_simp (Asymptotics.IsBigO _ _ _) := bigOSimprocImpl
...
elab "bigO" : tactic => withMainContext do
  -- Pre-normalization: distribute products, simplify nested powers, etc.
  -- This converts (n^2)^3 → n^6, (n^2+1)*log n → n^2*log n + log n, etc.
  try evalTactic (← `(tactic|
    conv_lhs => ext; simp only [add_mul, mul_add, one_mul, mul_one, ← pow_mul, pow_one]
    conv_rhs => ext; simp only [add_mul, mul_add, one_mul, mul_one, ← pow_mul, pow_one]))
  catch _ => pure ()

  -- Re-read goal after normalization
  let goal ← getMainGoal
  let goalType ← instantiateMVars (← goal.getType)

  match ← matchBigOGoal? goalType with
  | none =>
    -- Goal is not directly f =O[l] g; fall back to simp with bigO_simp
    -- to simplify/discharge any IsBigO sub-expressions within the goal
    evalTactic (← `(tactic| simp only [bigO_simp]))
    return
  | some (l, f, g) =>
  ...
  if gGrowth.terms.length ≤ 1 then
    -- Single-term RHS: direct proof
    buildProof fGrowth.terms gDom goal
  else
    -- Multi-term RHS: use transitivity f =O(dominant) =O(sum)
    let transConst ← mkConstWithFreshMVarLevels ``isBigO_trans_real
    let subgoals ← goal.apply transConst
    -- Separate Big-O subgoals from type class instance goals
    let mut bigOGoals : Array MVarId := #[]
    for sg in subgoals do
      if ← sg.isAssigned then continue
      let sgType ← instantiateMVars (← sg.getType)
    ...
    -- Solve first goal: f =O(?middle) — buildProof applies lemmas that
    -- will instantiate ?middle to the dominant term's concrete form.
    buildProof fGrowth.terms gDom bigOGoals[0]!
    -- Solve second goal: dominant =O(sum) — walk the addition tree
    -- using isBigO_left_add / isBigO_right_add with non-negativity proofs.
...
-- ═══════════════════════════════════════════════════════════════════
-- SIMPROC TESTS (bigO_simp: sub-expression simplification)
-- ═══════════════════════════════════════════════════════════════════

-- ══ Direct discharge via simproc (single-term both sides) ══
example : (fun n : ℝ => n ^ 2) =O[atTop] (fun n => n ^ 3) := by
  simp only [bigO_simp]

-- ══ LHS normalization (multi-term LHS → dominant) ══
example : (fun n : ℝ => n^3 + n^2 + n) =O[atTop] (fun n => n^4) := by
  simp only [bigO_simp]

-- ══ RHS normalization (multi-term RHS → dominant) ══
example : (fun n : ℝ => n^2) =O[atTop] (fun n => n^3 + n^2) := by
  simp only [bigO_simp]

-- ══ Both sides normalize ══
example : (fun n : ℝ => n^2 + n) =O[atTop] (fun n => n^3 + n^2) := by
  simp only [bigO_simp]

-- ══ bigO tactic fallback on non-bare-IsBigO goals ══
example (P : Prop) (hp : P) :
    P ∧ (fun n : ℝ => n^2 + n) =O[atTop] (fun n => n^3) := by
  constructor
  · exact hp
  · bigO

-- ══ simp discharges bigO inside conjunction ══
example (P Q : Prop) (hp : P) (hq : Q) :
    P ∧ ((fun n : ℝ => n^2 + n) =O[atTop] (fun n => n^3)) ∧ Q := by
  simp only [bigO_simp]
  -- Goal is now P ∧ True ∧ Q
  exact ⟨hp, trivial, hq⟩


-- Example: Big O with functions of functions, using the simproc (should NOT fail)
example :
  (fun n : ℝ => n^2)
    =O[atTop]
  (fun n => (n^2 + 1) * Real.log n) := by
  bigO
